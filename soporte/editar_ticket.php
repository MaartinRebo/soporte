...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $stmt = $conexion->prepare("UPDATE tickets SET nombre=?, usuario=?, tipoCaso=?, descripcion=?, resuelto=? WHERE id=?");
    $stmt->bind_param("ssssii", $nombre, $usuario, $tipoCaso, $descripcion, $resuelto, $id);
    $stmt->execute();
    $stmt->close();
...
<body class="container py-4">
    ...
    <form method="post">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="usuario" class="form-control" value="<?= htmlspecialchars($ticket['usuario']) ?>" required>
        </div>
        <div class="mb-3">
