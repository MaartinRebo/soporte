...
if (empty($form_data['correo'])) {
    ...
    $correos_validos = file('correos_validos.txt', FILE_IGNORE_NEW_LINES);
    if (!in_array($form_data['correo'], $correos_validos)) {
        $errores['correo'] = 'El correo no está autorizado.';
    }
}
...
$stmt = $mysqli->prepare("INSERT INTO tickets (nombre, correo, tipo_caso, descripcion, archivo_nombre, archivo_tipo, archivo_datos) VALUES (?, ?, ?, ?, ?, ?, ?)");
$stmt->bind_param(
    "sssssss",
    $form_data['nombre'],
    $form_data['correo'],
